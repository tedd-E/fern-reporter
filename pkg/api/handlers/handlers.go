...
func GetLongestTestRuns(h *Handler, projectName string) []models.TestRunInsight {
	...
	h.db.Table("test_runs").
		...
		Select("suite_runs.id, test_runs.test_project_name, "+
		...
		Group("suite_runs.id, test_runs.test_project_name, test_duration").
		Find(&testRuns)
